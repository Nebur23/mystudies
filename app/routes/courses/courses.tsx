...
export default function CoursesPage() {
  ...
  const [optimisticFilter, setOptimisticFilter] = useState(currentFilters.filter || null);
  const navigation = useNavigation();

  const currentLoadedFilter = currentFilters.filter || "all";
  ...
  const isSwitchingFilter =
    ...
    nextFilter !== currentLoadedFilter;

  const [showShimmer, setShowShimmer] = useState(false);
  const isPending = showShimmer;

  // Prevent instant flash for very fast responses
  useEffect(() => {
    ...
    if (isSwitchingFilter) {
      ...
      // IMPORTANT:
      // wait one paint AFTER new loader data committed
  ...
  }, [isSwitchingFilter, currentLoadedFilter]);


  console.log("filter", optimisticFilter, currentFilters.filter);

  useEffect(() => {
    setOptimisticFilter(currentFilters.filter || null);
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-lg mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-2 mt-3 overflow-x-auto pb-1 scrollbar-hide">
            {/* Quick Filter Chips */}
            {[
              { key: "filter", label: "All", value: "all" },
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${optimisticFilter === chip.value
      ...
      {isPending ? (
        <div className="max-w-lg mx-auto px-4 py-3">
          <div className="flex gap-3 text-xs text-slate-600 animate-pulse">
            <span className="flex items-center gap-1">  
              <BookOpen size={14} /> <div className="h-3 w-6 bg-slate-200 rounded" />
            </span>
            <span className="w-1 h-1 bg-slate-300 rounded-full" />
            <span className="flex items-center gap-1">
              <Clock size={14} /> <div className="h-3 w-6 bg-slate-200 rounded" />
            </span>
            <span className="w-1 h-1 bg-slate-300 rounded-full" />
            <span className="flex items-center gap-1">
              <Trophy size={14} /> <div className="h-3 w-6 bg-slate-200 rounded" />
            </span>
          </div>
        </div>
      ) : (
      ...
      <main className="max-w-lg mx-auto px-4 py-2">
        {showShimmer ? (
          ...
          <div className="text-center py-12">
            ...
            <button
              onClick={() => { setSearchParams(new URLSearchParams()); setLocalSearch(""); }}
